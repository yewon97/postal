<?php
  ...
  $ch = curl_init();
  $url = 'https://api.odcloud.kr/api/15070368/v1/uddi:cea8854d-35c4-4a7f-a100-f241ea289d76?page='.$page.'&perPage=10&returnType=XML&serviceKey=2MlX7gbD3hRfwbKnnAd8WXgMGGFflDzdbYF%2BpFZbV0ys%2F3aq%2FoEWmEcb5ZE4Xqz6fOYAh7F1tqO0Z%2FO9KQ%2FmGA%3D%3D'; 
...
<body>
  ...
  <main id="main" class="main">
    ...
    <section id="tab1" class="cont_container">
      ...
      <table summary="등록된 공지사항" class="table">
        ...
        <tbody>
        <?php
          for ($i=0;$i<sizeof($xml->data->item);$i++) {
            $zoneCenter = $xml->xpath("//data/item/col[@name='총괄국명']")[$i];
            $postalName = $xml->xpath("//data/item/col[@name='우체국명']")[$i];
            $address = $xml->xpath("//data/item/col[@name='주소(도로명)']")[$i];
            $phone = $xml->xpath("//data/item/col[@name='전화번호']")[$i];
        ?>
        <tr>
          <!-- $print_num = strval($page-1).strval($i+1); -->
          <td><?=strval($page-1).$i+1?></td>
          <td><?=$zoneCenter?></td>
          <td><?=$postalName?></td>
          <td><?=$address?></td>
          <td><?=$phone?></td>
